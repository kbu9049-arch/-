...
애플 웹사이트의 짜임새를 기준으로 삼았습니다. 화면을 끌고 가는 것은 장식이 아니라
글자이고, 크기·굵기·여백만으로 위계를 만듭니다. 큰 제목에도 자간을 과하게 좁히지
않고(56px 에서 -.005em), 그러데이션·발광·색 그림자는 쓰지 않으며, 경계는 실선
한 줄로 긋습니다. 움직임은 나타남과 약간의 위 이동뿐입니다.
애플 기기에서는 그 기기의 서체(SF Pro / Apple SD Gothic Neo)를 그대로 쓰고,
그 밖에서는 Pretendard 가 같은 자리를 메웁니다.
