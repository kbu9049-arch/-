...
apple.com/kr 의 구성을 그대로 빌려 왔습니다. 화면은 전폭으로 깔리는 흰색·연회색·검정
띠가 위아래로 쌓인 구조이고, 글은 그 안의 980px 폭에만 놓입니다. 띠마다 가운데 정렬된
큰 제목 하나와 한 줄 설명, 파란 "… 보기 >" 링크가 붙습니다. 상단 바는 44px 로 얇고
반투명하며, 성분·효능 상세 화면에는 그 아래에 이름과 구간 링크가 붙은 두 번째 바가
하나 더 붙습니다. 큰 숫자는 검정 띠 위에 아주 크게 놓고 카드로 감싸지 않습니다.
홈 화면 맨 아래에는 애플 홈처럼 2열 타일이 있습니다.

화면을 끌고 가는 것은 장식이 아니라 글자이고, 크기·굵기·여백만으로 위계를 만듭니다.
큰 제목에도 자간을 과하게 좁히지 않고, 그러데이션·발광·색 그림자는 쓰지 않으며,
경계는 실선 한 줄로 긋습니다. 움직임은 나타남과 약간의 위 이동, 숫자가 세어
올라가는 것뿐입니다. 애플 기기에서는 그 기기의 서체(SF Pro / Apple SD Gothic Neo)를
그대로 쓰고, 그 밖에서는 Pretendard 가 같은 자리를 메웁니다.

### 색 체계

화면은 거의 흑백입니다. 색은 딱 두 군데만 씁니다.

| | 어디에 | 뜻이 있나 | 검사 |
|---|---|---|---|
| **결과 방향 3색** (`--sig`/`--null`/`--unclear`) | 결과 막대, 범례, 논문의 결과 라벨 | 있음 — 차이 있었음 / 없었음 / 알 수 없음 | 색각 이상에서도 최소 ΔE 24.7(라이트)·26.8(다크). 그래도 막대마다 숫자와 글자 라벨을 함께 둔다 |
| **성분 분류 12종** | 분류 이름이 적혀 있는 자리에만 — 분류 칩, 목록의 분류 이름표, 상세 화면 머리말 | 없음 — 그냥 이름표 | 열두 가지를 색만으로 구별되게 만들 수는 없다(색각 이상에서 최소 ΔE 1.4). 그래서 색에 뜻을 싣지 않고, 이름표에 언제나 한글 이름을 붙인다 |

히어로·수치 타일·효능 타일·버튼·상단 바에는 색을 넣지 않습니다. 측정 항목의 계통
묶음(30 → 8)도 이름만 글자로 보여 주고 색은 주지 않습니다.

색은 `scripts/gen_colors.py` 가 OKLCH(사람 눈이 느끼는 밝기 기준 색 공간)에서
계산해 `web/colors.css` 로 내보냅니다. 밝기·진하기를 거의 고정하고 색상만 돌려
뽑기 때문에 어느 분류를 골라도 화면에서 느껴지는 무게가 같습니다.
글자로 쓰는 색은 제 바탕 대비 4.5:1 이상, 채워 쓰는 색은 3:1 이상을 스크립트가
직접 재서 미달이면 밝기를 옮깁니다. `make site` 와 배포 워크플로가 매번 다시
계산하므로 손으로 고칠 일이 없습니다.
