...
# ── 연구 유형 ────────────────────────────────────────────────────────────────
# (id, 한글 표기, 근거 위계 점수) — 점수는 '대표 논문' 정렬에만 사용한다.
STUDY_TYPES = {
    "meta_analysis":     ("메타분석", 6),
    "systematic_review": ("체계적 문헌고찰", 5),
    "rct":               ("무작위대조시험(RCT)", 4),
    "clinical_trial":    ("임상시험(비무작위)", 3),
    "observational":     ("관찰연구", 2),
    "review":            ("종설", 1),
    "preclinical":       ("동물·시험관 연구", 0),
    "other":             ("기타", 0),
}
...
DIRECTIONS = {
    "significant": "유의한 결과 보고",
    "null": "유의차 없음",
    "unclear": "판정 불가",
}

# ── 쉬운 말 ─────────────────────────────────────────────────────────────────
# 화면에 기본으로 보여줄 표현과, 눌렀을 때 나오는 설명.
# 연구 용어를 모르는 사람이 읽어도 무엇을 뜻하는지 알 수 있어야 한다.
# (label, 설명) — label 이 비어 있으면 위쪽 표의 정식 명칭을 그대로 쓴다.
PLAIN_STUDY_TYPES = {
    "meta_analysis": ("메타분석",
        "같은 주제를 다룬 여러 연구의 결과를 통계로 하나로 합친 연구입니다. "
        "연구 하나보다 훨씬 믿을 만한 근거로 봅니다."),
    "systematic_review": ("문헌 종합 검토",
        "정해진 기준에 맞는 논문을 빠짐없이 찾아 모아 정리한 연구입니다."),
    "rct": ("무작위 비교 시험",
        "참가자를 제비뽑기하듯 두 무리로 나눠 한쪽에만 그 성분을 주고 비교한 시험입니다. "
        "사람을 대상으로 한 연구 중에서는 가장 믿을 만한 방식입니다."),
    "clinical_trial": ("사람 대상 시험",
        "사람에게 직접 먹여 본 시험입니다. 다만 무작위로 나누지 않아 "
        "무작위 비교 시험보다는 근거가 약합니다."),
    "observational": ("관찰 연구",
        "원래 그렇게 먹고 있던 사람들을 지켜본 연구입니다. "
        "먹어서 그렇게 된 건지, 원래 그런 사람이 먹은 건지 가리기 어렵습니다."),
    "review": ("해설 글",
        "다른 연구들을 소개하고 설명한 글입니다. 새로 실험한 결과가 아닙니다."),
    "preclinical": ("동물·세포 실험",
        "사람이 아니라 쥐 같은 동물이나 배양한 세포로 한 실험입니다. "
        "사람에게 같은 결과가 나온다는 보장은 없습니다."),
    "other": ("기타",
        "위 분류에 들어가지 않는 논문입니다."),
}

PLAIN_SUBJECTS = {
    "human": ("사람에게 실험", "사람을 대상으로 한 연구입니다."),
    "animal": ("동물에게 실험",
        "쥐 같은 동물로 한 실험입니다. 사람에게 같은 결과가 나온다는 보장은 없습니다."),
    "invitro": ("세포·시험관 실험",
        "배양 접시나 시험관에서 한 실험입니다. 사람에게 같은 결과가 나온다는 보장은 없습니다."),
    "unknown": ("대상 불명", "무엇을 대상으로 한 연구인지 초록에서 확인되지 않았습니다."),
}

PLAIN_DIRECTIONS = {
    "significant": ("차이 있었음",
        "가짜약(위약)이나 비교 대상과 견줘 통계적으로 의미 있는 차이가 있었다고 "
        "그 논문이 보고했습니다. '몸에 좋아졌다'는 뜻은 아닙니다 — "
        "어느 쪽으로 움직였는지까지는 이 도구가 판정하지 않습니다."),
    "null": ("차이 없었음",
        "가짜약이나 비교 대상과 견줘 의미 있는 차이가 나타나지 않았습니다."),
    "unclear": ("알 수 없음",
        "논문 초록에 적힌 문장만으로는 차이가 있었는지 없었는지 가릴 수 없었습니다."),
}


def plain_table() -> dict:
    """화면에 내려보낼 쉬운 말 표. {종류: {코드: {label, term, desc}}}"""
    def pack(plain_map, formal_map):
        return {k: {"label": v[0], "term": formal_map.get(k, v[0]), "desc": v[1]}
                for k, v in plain_map.items()}
    return {
        "study_type": pack(PLAIN_STUDY_TYPES, {k: v[0] for k, v in STUDY_TYPES.items()}),
        "subject": pack(PLAIN_SUBJECTS, SUBJECTS),
        "direction": pack(PLAIN_DIRECTIONS, DIRECTIONS),
    }
